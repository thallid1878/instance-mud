  ...
  if (CONFIG_PK_SETTING == CONFIG_PK_OFF) {
	...
	!PLR_FLAGGED(victim, PLR_KILLER)) {
      ...
      mudlog(BRF, MAX(LVL_IMMORT, MAX(GET_INVIS_LEV(ch), GET_INVIS_LEV(victim))), TRUE, 
        "%s failed summoning %s to %s.", GET_NAME(ch), GET_NAME(victim), GET_ROOM(ch)->name);
      return;
    }
  }
...
ASPELL(spell_identify)
{
  ...
  if (obj) {
    ...
    send_to_char(ch, "Armor: %d, Hitroll: %d, Damroll: %d\r\n", compute_armor_value(victim), GET_HITROLL(victim), GET_DAMROLL(victim));
